...
interface DineInOrderListProps {
  refreshTrigger: boolean;
}

export default function DineInOrderList({
  refreshTrigger,
}: DineInOrderListProps) {
  ...
  const loadData = async () => {
    try {
      ...
      const dineInOrders = ordersData.filter(
        (order: Order) => order.type === "dine-in"
      );
  ...
  if (loading)
    return <div className="mt-4">Đang tải danh sách đơn hàng tại quán...</div>;

  return (
    <div className="mt-8">
      <h4 className="text-lg font-semibold mb-4">
        Danh sách đơn hàng tại quán
      </h4>
      {orders.length === 0 ? (
        <p>Chưa có đơn hàng tại quán nào.</p>
      ) : (
